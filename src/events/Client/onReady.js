...
const { checkDocument, updateDocument } = require('../../utils/mongodb');
    ...
    run: async (__client__, client) => {
        try {
            ...
            if (channel && channel.isTextBased()) {
                const { embed, rows } = createInterface();

                let dashboardMessage;
                if (settings.vc_dashboard_message) {
                    try {
                        dashboardMessage = await channel.messages.fetch(settings.vc_dashboard_message);
                    } catch {
                        dashboardMessage = null; // message not found or deleted
                    }
                }
                ...
                if (dashboardMessage) {
                    ...
                    // Save new message ID in Mongo
                    await updateDocument(
                        'voice_channels',
                        { _id: guildId },
                        { $set: { vc_dashboard_message: sent.id } }
                    );
                }
            } else {
                console.error(
                    `Channel with ID "${settings.vc_dashboard}" not found or is not a text channel.`
                );
            }
